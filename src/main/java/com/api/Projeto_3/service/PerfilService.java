...
public class PerfilService implements UserDetailsService {
    ...
    public PerfilsDtos buscarTreinadorPorId(Long id) {
        TreinadorModel treinador = jpaTreinador.findById(id)
                .orElseThrow(() -> new RuntimeException("Treinador não encontrado com ID: " + id));
        return new PerfilsDtos(treinador);
    }
    ...
    private void salvarPerfil(PerfilsDtos dos, PerfisModelo atl) {
        RolesModel roleReal = roles.findById(dos.getRoles().getId())
